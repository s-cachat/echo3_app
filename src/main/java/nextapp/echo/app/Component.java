 ...
 * <p>
 * A hierarchy of <code>Component</code> objects is used to represent the state
 * of an application's user interface. A <code>Component</code> may have a
 * single parent <code>Component</code> and may contain zero or more child
 * <code>Component</code>s. Certain <code>Component</code>s may limit the number
 * or type(s) of children which may be added to them, and may even establish
 * requirements for what type(s) of parent <code>Component</code>s they may be
 * added to. In the event that an application attempts to add a child
 * <code>Component</code> to a parent <code>Component</code> in spite of these
 * requirements, an <code>IllegalChildException</code> is thrown.
 *
 ...
 * contained in a relevant <code>Style</code> or <code>StyleSheet</code> will be
 * used for rendering when the property values are not specified by a
 ...
 * <code>PROPERTY_LAYOUT_DATA</code>. The rendering application container will
 * use the <code>Component.getRenderProperty()</code> and
 * <code>Component.getRenderIndexedProperty()</code> to retrieve the values of
 * stylistic properties, in order that their values might be obtained from the
 * <code>Component</code>'s shared <code>Style</code> or the
 * <code>ApplicationInstance</code>'s <code>StyleSheet</code> in the event they
 * are not directly set in the <code>Component</code>.
 * <p>
 * A <code>Component</code> implementation should not store the values of style
 * properties as instance variables. Rather, the values of style properties
 * should be stored in the local <code>Style</code> instance, by way of the
 * <code>set()</code> method. The <code>get()</code> method may be used to
 * obtain the value of such properties. Only style properties should be stored
 * using these methods; properties such as models should never be stored using
 * the <code>get()</code>/<code>set()</code> interface.
 *
 * <h3>Events</h3>
 * <p>
 * Many <code>Component</code>s will provide the capability to register
 * <code>EventListener</code>s to notify interested parties when various state
 * changes occur. The base <code>Component</code> class provides an
 * <code>EventListenerList</code> as a convenient and memory efficient means of
 * storing such listeners. The internal <code>EventListenerList</code> may be
 * obtained using the <code>getEventListenerList()</code> method. The
 * <code>EventListenerList</code> is lazy-created and will only be instantiated
 * on the first invocation of the <code>getEventListenerList()</code> method. If
 * the intent is only to inquire about the state of event listeners without
 * necessarily forcing instantiation of an <code>EventListenerList</code>, the
...
public abstract class Component
    ...
    /**
     * Flag indicating the <code>Component</code> is currently in the process of
     * being disposed.
     */
    ...
    /**
     * Property change event name for immediate children being made
     * visible/invisible. When used, the <code>newValue</code> of the event will
     * represent a child made visible, OR the <code>oldValue</code> will
     * represent a child made invisible.
     */
     ...
     * When used, the <code>newValue</code> of the event will represent an added
     * child, OR the <code>oldValue</code> will represent a removed child.
     */
    ...
    /**
     * Property change event name for previous focus traversal component
     * changes.
     */
    ...
    /**
     * Verifies a character can be used as a non-initial character in a
     * renderId. Character must be a (7-bit ASCII) letter, digit, or underscore.
     *
     * @param ch the character to verify
     * @return true if the character is a (7-bit ASCII) letter, digit, or
     * underscore.
     */
    ...
    /**
     * The <code>ApplicationInstance</code> to which the component is
     * registered.
     */
    private ApplicationInstance applicationInstance;

    /**
     * An ordered collection of references to child components. This object is
     * lazily instantiated.
     */
     ...
     * Multiple booleans are wrapped in a single integer for memory use
     * reduction.
     */
    private int flags;

    /**
     * A user-defined identifier for this component. This identifier is not
     * related in any way to <code>renderId</code>.
     */
    private String id;

    /**
     * The layout direction of the component. This property is generally unset,
     * as layout direction information is normally inherited from the
     * <code>ApplicationInstance</code> or from an ancestor
    ...
    /**
     * The locale of the component. This property is generally unset, as locale
     * information is normally inherited from the
     * <code>ApplicationInstance</code> or from an ancestor
     * <code>Component</code> in the hierarchy.
     */
    ...
    /**
     * A application-wide unique identifier for this component. This identifier
     * is not related in any way to <code>id</code>.
     */
     ...
     * <code>Component</code>. The child will be added at the greatest index.
     *
    ...
    /**
     * Adds the specified <code>Component</code> as the <code>n</code>th child
     * of this component. All component-add operations use this method to add
     * components. <code>Component</code>s that require notification of all
     * child additions should override this method (making sure to call the
     * superclass' implementation). If the child component currently has a
     * parent in another hierarchy, it will automatically be removed from that
     * hierarchy before being added to this component. This behavior will also
     * occur if the child component is currently a child of this component.
     *
     ...
     * @throws IllegalChildException if the child is not allowed to be added to
     * this component, because it is either not valid for the component's state
    ...
    /**
     * Adds a property change listener to this <code>Component</code> for a
     * specific property.
     *
    ...
    /**
     * Internal method to set the render identifier of
     * the<code>Component</code>. This method is invoked by the
     * <code>ApplicationInstance</code> when the component is registered or
     * unregistered, or by manual invocation of <code>setRenderId()</code>. This
     * method performs no error checking.
     *
    ...
    /**
     * Life-cycle method invoked when the <code>Component</code> is removed from
     * a registered hierarchy. Implementations should always invoke
     * <code>super.dispose()</code>. Modifications to the component hierarchy
     * are not allowed within this method.
     */
    public void dispose() {
    }
    ...
    /**
     * Recursively executes the <code>init()</code> life-cycle methods of this
     * <code>Component</code> and its descendants.
     */
    void doInit() {
        if (applicationInstance == null) {
            return;
    ...
    /**
     * Returns the value of the specified property. This method is generally
     * used only internally by a <code>Component</code>, however there are
     * exceptions. The more specific <code>getXXX()</code> methods to retrieve
     * property values from a <code>Component</code> whenever possible. See the
     * class-level documentation for a more detailed explanation of the use of
     * this method.
     *
     ...
     * This property may not be relevant to certain components, though even in
     * such cases may be useful for setting a default for children.
     *
    ...
    /**
     * Recursively searches for the component with the specified id by querying
     * this component and its descendants. The id value is that retrieved and
     * set via the <code>getId()</code> and <code>setId()</code> methods. This
     * method is in no way related to <code>renderId</code>s.
     *
    ...
    /**
     * Returns the default/base font of the component. This property may not be
     * relevant to certain components, though even in such cases may be useful
     * for setting a default for children.
     *
     * @return the font
     */
    public Font getFont() {
    ...
    /**
     * Returns the user-defined identifier of the <code>Component</code>. Note
     * that the user defined identifier has no relation to the
     * <code>renderId</code>.
     *
    ...
    /**
     * Returns the value of the specified indexed property. This method is
     * generally used only internally by a <code>Component</code>, however there
     * are exceptions. The more specific <code>getXXX()</code> methods to
     * retrieve property values from a <code>Component</code> whenever possible.
     * See the class-level documentation for a more detailed explanation of the
     * use of this method.
     *
     * @param propertyName the property name
    ...
    /**
     * Returns the specific locale setting of this component, if any. This
     * method will return null unless a <code>Locale</code> is specifically set
     * on <strong>this</strong> <code>Component</code>.
     *
    ...
    /**
     * Returns the <code>Style</code> object in which local style properties are
     * stored. Access to this object is provided solely for the purpose of
     * allowing the enabling the application container to render the state of
    ...
    /**
     * Returns the render id of this component. This id is only guaranteed to be
     * unique within the <code>ApplicationInstance</code> to which this
     * component is registered. This method returns null in the event that the
    ...
    /**
     * Determines the &quot;rendered state&quot; of an indexed property. The
     * rendered state is determined by first determining if the given property
     * is locally set on this <code>Component</code>, and returning it in that
     * case. If the property state is not set locally, the shared
     * <code>Style</code> assigned to this component will be queried for the
     * property value. If the property state is not set in the shared
     * <code>Style</code>, the <code>StyleSheet</code> of the
     * <code>ApplicationInstance</code> to which this <code>Component</code> is
     * registered will be queried for the property value. In the event the
     * property is not set in any of these resources, null is returned.
     * <p>
     * The application container will invoke this method rather than individual
     * property getter methods to determine the state of properties when
     * rendering.
     *
     ...
     * <code>ApplicationInstance</code> to which this <code>Component</code> is
     * registered will be queried for the property value. In the event the
     * property is not set in any of these resources, <code>defaultValue</code>
     * is returned.
     *
     * @param propertyName the name of the property
     ...
     * will be returned. In the event that no locale information is available
     * from the ancestral hierarchy of <code>Component</code>s and no
    ...
    /**
     * Determines the &quot;rendered state&quot; of a property. The rendered
     * state is determined by first determining if the given property is locally
     * set on this <code>Component</code>, and returning it in that case. If the
     * property state is not set locally, the shared <code>Style</code> assigned
     * to this component will be queried for the property value. If the property
     * state is not set in the shared <code>Style</code>, the
     * <code>StyleSheet</code> of the <code>ApplicationInstance</code> to which
     * this <code>Component</code> is registered will be queried for the
     * property value. In the event the property is not set in any of these
     ...
     * this <code>Component</code> is registered will be queried for the
     * property value. In the event the property is not set in any of these
     * resources, <code>defaultValue</code> is returned.
     *
     * @param propertyName the name of the property
     * @param defaultValue the value to be returned if the property is not set
     * @return the property state
     ...
     * <code>Component</code>. As its name implies, the <strong>shared</strong>
     * <code>Style</code> may be shared amongst multiple
     * <code>Component</code>s. Style properties will be rendered from the
     * specified <code>Style</code> when they are not specified locally in the
     ...
     * <code>ApplicationInstance</code>'s<code>StyleSheet</code> from which the
     * renderer will retrieve properties. The renderer will only query the
     * <code>StyleSheet</code> when properties are not specified directly by the
     * <code>Component</code> or by the <code>Component</code>'s shared
     * <code>Style</code>.
     *
    ...
    /**
     * Determines if a local <code>EventListenerList</code> exists. If no
     * listener list exists, it can be assured that there are thus no listeners
     * registered to it. This method should be invoked by event firing code
     * prior to invoking <code>getListenerList()</code> to avoid unnecessary
     * creation of an <code>EventListenerList</code> in response to their query.
     *
    ...
    /**
     * Life-cycle method invoked when the <code>Component</code> is added to a
     * registered hierarchy. Implementations should always invoke
     * <code>super.init()</code>. Modifications to the component hierarchy are
     * not allowed within this method.
     */
    public void init() {
    }

    /**
     * Determines if this <code>Component</code> is or is an ancestor of the
    ...
    /**
     * @deprecated use focusNextId/focusPreviousId in (rare) scenario where
     * focus of a specific component is to be avoided. This method will be
     * removed soon.
     */
    @Deprecated
    ...
    /**
     * Determines whether this <code>Component</code> should be rendered with an
     * enabled state. Disabled<code>Component</code>s are not eligible to
     * receive user input. The application container may render disabled
     * components with an altered appearance.
     *
    ...
    /**
     * Determines if this <code>Component</code> is valid to be added as a child
     * of the given parent <code>Component</code>. Default implementation always
     * returns true, may be overridden to provide specific behavior.
     *
    ...
    /**
     * Returns the visibility state of this <code>Component</code>. Non-visible
     * components will not be seen by the rendering application container, and
     * will not be rendered in any fashion on the user interface. Rendering
     * Application Containers should ensure that no information about the state
     * of an invisible component is provided to the user interface for security
     * purposes.
     *
    ...
    /**
     * Processes client input specific to the <code>Component</code> received
     * from the <code>UpdateManager</code>. Derivative implementations should
     * take care to invoke <code>super.processInput()</code>.
     *
     * <strong>Security note:</strong> Because input to this method is likely
     * from a remote client, it should be treated as potentially hostile. All
     * input to this method should be carefully verified. For example, directly
     * invoking <code>set()</code> method with the provided input would
     ...
     * <code>dispose()</code> operation or if an attempt is made to remove a
     * <code>Component</code> during an <code>init()</code> operation.
     */
     ...
     * <code>Component</code>.
     * <p>
     * All <code>Component</code> remove operations use this method to remove
     * <code>Component</code>s. <code>Component</code>s that require
     * notification of all child removals should override this method (while
     * ensuring to call the superclass' implementation).
     *
    ...
    /**
     * Removes a property change listener from this <code>Component</code> for a
     * specific property.
     *
     * @param propertyName the name of the property for which to listen
     * @param l the listener to be removed
    ...
    /**
     * Sets a generic property of the <code>Component</code>. The value will be
     * stored in this <code>Component</code>'s local style.
     *
     * @param propertyName the name of the property
    ...
    /**
     * Sets the child components for this container, removing any existing
     * children.
     *
    ...
    public void setEnabled(boolean newValue) {
        ...
        if (oldValue != newValue) {
            ...
            firePropertyChange(ENABLED_CHANGED_PROPERTY, oldValue, newValue);
        }
    }
    ...
    /**
     * Sets the next focusable component. RenderIds are used for setting focus
     * traversal order in order to avoid referencing/garbage collection issues.
     * Ensure that the component has a renderId (either by way of it having been
     * registered with the application, or having it manually set).
     *
    ...
    /**
     * Sets the previous focusable component. RenderIds are used for setting
     * focus traversal order in order to avoid referencing/garbage collection
     * issues. Ensure that the component has a renderId (either by way of it
     * having been registered with the application, or having it manually set).
     *
     * @param newValue the <code>renderId</code> of the previous focusable
     * component
     */
    ...
    /**
     * Sets a generic indexed property of the <code>Component</code>. The value
     * will be stored in this <code>Component</code>'s local style.
     *
     * @param propertyName the name of the property
    ...
    /**
     * Sets the <code>LayoutData</code> of this <code>Component</code>. A
     * <code>LayoutData</code> implementation describes how this
     * <code>Component</code> is laid out within/interacts with its containing
    ...
    /**
     * Sets a custom render identifier for this <code>Component</code>. The
     * identifier may be changed without notification if another component is
     * already using it. Identifiers are limited to ASCII alphanumeric values.
     * The first character must be an upper- or lower-case ASCII letter.
     * Underscores and other punctuation characters are not permitted. Use of
     * "TitleCase" or "camelCase" is recommended.
     *
     * @param renderId the new identifier
     */
    ...
    /**
     * Sets the shared style of the <code>Component</code>. Setting the shared
     * style will have no impact on the local stylistic properties of the
     * <code>Component</code>.
     *
     ...
     * <code>ApplicationInstance</code>-defined <code>StyleSheet</code>. Setting
     * the style name will have no impact on the local stylistic properties of
     ...
     * implementations should ensure to invoke <code>super.validate()</code> out
     * of convention.
     */
     ...
     * enabled, and not "obscured" by the presence of any modal component. If
     * overriding this method, your implementation should invoke
     * <code>super.verifyInput()</code>.
     *
     * @param inputName the name of the input
     * @param inputValue the value of the input
     * @return true if the input is allowed to be processed by this component in
     * its current state
     */
     ...
     * given <code>Component</code> is not a child, <code>-1</code> is returned.
     *
     * @param c the <code>Component</code> to analyze
     * @return the index of the specified <code>Component</code> amongst the
